#!/usr/bin/env python3
"""backfill_djia.py — 一次性向前追溯最近约 5 个月（160 交易日）的
道琼斯 (Dow Jones) QDII ETF 价格 / 净值 / 折溢价历史，并做双数据源交叉校验。
...
产出：
  - djia_all.json            （含 sh513400 的 5 个月 price / nav / premium）
  - data/djia_data_YYYY-MM.js（逐月拆分，供前端按现有约定加载）
  - index.html               （在美国50数据块之后注入 <script> 引用）
  - djia_backfill_report.json（双源对比报告，含最大溢价差、超阈值点数）
"""
import os
import re
import json
...
DJIA_META = {
    'sh513400': {
        ...
        'fee_total': 0.65, 'fee_mgmt': 0.50, 'fee_cust': 0.15, 'fee_svc': 0.00,
        'size': '36.86',  # 上交所 2026-09-03 最新规模 36.8623 亿元
    },
}
...
def backfill():
    ...
    print(f'DJIA backfill: {start_date} ~ {today.strftime("%Y-%m-%d")} '
    ...
    for code, meta in DJIA_META.items():  # noqa
        print(f'\n>>> {code} ({meta["name"]})')
    ...
    # 组内（道琼斯单只）按最新共同净值日归一化，与每日更新逻辑一致
    du.normalize_group_premiums(all_data, list(DJIA_META.keys()))

    json_path = os.path.join(du.DIR, 'djia_all.json')
    du.write_text_atomic(json_path, json.dumps(all_data, ensure_ascii=False))
    print(f'\n[OK] wrote {json_path}')

    _write_monthly(all_data)

    rep_path = os.path.join(du.DIR, 'djia_backfill_report.json')
...
def _write_monthly(all_data):
    ...
    for month in sorted(months):
        ...
        if not any(subset[c]['price'] for c in subset):
            continue
        js_path = os.path.join(du.DATA_DIR, f'djia_data_{month}.js')
        var_name = f'DJIA_DATA_{month.replace("-", "")}'
...
def _inject_html_scripts(months):
    ...
    with open(html_path, 'r', encoding='utf-8') as f:
        html = f.read()
    pattern = re.compile(r'<script src="data/us50_data_(\d{4}-\d{2})\.js"></script>')
    matches = list(pattern.finditer(html))
    if not matches:
        raise RuntimeError('cannot find us50 script anchor in index.html')
    anchor_end = matches[-1].end()
    new_tags = ''
    for m in months:
        tag = f'<script src="data/djia_data_{m}.js"></script>'
        if tag not in html:
            new_tags += '\n' + tag
    if new_tags:
        html = html[:anchor_end] + new_tags + html[anchor_end:]
        du.write_text_atomic(html_path, html)
        print(f'  [HTML] injected {len(months)} djia script tag(s) after us50 block')
